...
def neuropil_subtraction(mov: np.ndarray, filter_size: int) -> None:
    ...
    for frame, framet in zip(mov, movt, strict=False):
...
def sparsery(
    ...
    for movu0, gxy0, I0 in zip(movu, gxy, I, strict=False):
        gmodel = RectBivariateSpline(
            gxy0[1, :, 0], gxy0[0, 0, :], movu0.max(axis=0), kx=min(3, gxy0.shape[1] - 1), ky=min(3, gxy0.shape[2] - 1)
        )
    ...
    message = (
        f"Plane {plane_number} sparse detection parameters: {estimate_mode.value} spatial scale "
        f"~{int(spatscale_pix)} pixels, time epochs {round(vmultiplier, 2)}, threshold {round(vmultiplier * Th2, 2)} "
    )
    ...
    for tj in range(max_iterations):
        ...
        for j in range(nscales):
            ...
            V1[j][ys[j], xs[j]] = (Mx**2 * np.float32(Mx > threshold)).sum(axis=0) ** 0.5
    ...
    message = f"Found {tj} plane {plane_number} ROIs with score {round(v_max[tj], 2)}."
