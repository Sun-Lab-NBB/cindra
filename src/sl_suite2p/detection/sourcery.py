...
def getSVDproj(mov: np.ndarray, ops, u):
    ...
    nbins, Lyc, Lxc = np.shape(mov)
    if ops.get("smooth_masks"):
        sig = np.maximum([0.5, 0.5], ops["diameter"] / 20.0)
        for j in range(nbins):
            mov[j, :, :] = gaussian_filter(mov[j, :, :], sig)
    if 1:
        sdmov = standard_deviation_over_time(mov, batch_size=ops["batch_size"])
...
def localMax(V, footprint, thres):
    ...
    imax = np.maximum(thres, maxV - 1e-10) < V
    i, j = imax.nonzero()
    i = i.astype(np.int32)
    j = j.astype(np.int32)
    return i, j
...
def pairwiseDistance(y, x):
    dists = (
        (np.expand_dims(y, axis=-1) - np.expand_dims(y, axis=0)) ** 2
...
def get_stat(ops, stats, Ucell, codes, frac=0.5):
    ...
    for k, (stat, code) in enumerate(zip(stats, codes, strict=False)):
        ...
        if "radius" not in stat:
            ...
            stat["aspect_ratio"] = 2 * ry / (0.01 + ry + rx)
    ...
    for stat, footprint in zip(stats, footprints, strict=False):
    ...
    for stat, npix0 in zip(stats, npix, strict=False):
...
def getVmap(Ucell, sig):
    us = gaussian_filter(Ucell, [sig[0], sig[1], 0.0], mode="wrap")
...
def iter_extend(ypix, xpix, Ucell, code, refine=-1, change_codes=False):
    ...
    while npix < 10000:
        ...
        lam = lam / np.sum(lam**2 + 1e-10) ** 0.5
...
def sourcery(mov: np.ndarray, ops, plane_number: int):
    change_codes = True
    ...
    while 1:
        ...
        message = f"Found {ncells} plane {plane_number} ROIs with cost {err}. Time taken {timer.elapsed}."
