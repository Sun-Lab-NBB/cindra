# PIRT Documentation and Code Style Guide

This guide captures the documentation and coding conventions used in the pirt package. Reference this during future development sessions to maintain consistency.
...
- Always specify dtype: `NDArray[np.float32]`, not just `NDArray`
- Use `TYPE_CHECKING` block for `NDArray` to avoid runtime import overhead
- Standard precision is **float32** for all arrays (fields, knots, weights, coefficients)
...
| `gy`, `gx` | `knot_index_y`, `knot_index_x` |

### Functions

- Use descriptive verb phrases: `compute_basis_coefficients`, `fit_knots_to_field`
- Private functions start with underscore: `_sample_grid`, `_freeze_edges`
...
compute_basis_coefficients(interpolation_factor=t, coefficients=out)
self._get_knots(dimension=0)
_sample_grid(result=field_y, grid_sampling=self._grid_sampling, knots=knots)

# Avoid
np.zeros((4,), np.float32)
compute_basis_coefficients(t, out)
self._get_knots(0)
```
...
def set_from_fields(self, fields: tuple[NDArray[np.float32], NDArray[np.float32]]) -> None:
    if not (0 < injective_factor <= 1):
        message = (
...
def _fit_knots_to_field(...) -> None:
    ...

# Parallelized function
@numba.njit(cache=True, parallel=True)
def _sample_grid(...) -> None:
    for y in prange(result.shape[0]):  # Parallel outer loop
        for x in range(result.shape[1]):  # Sequential inner loop
            ...

# Inlined helper (for small, frequently-called functions)
@numba.njit(cache=True, inline="always")
def compute_basis_coefficients(...) -> None:
    ...
```
...
- Use `inline="always"` for small coefficient computation functions
...
def deformation_fields(self) -> tuple[NDArray[np.float32], NDArray[np.float32]]:
    """Returns two arrays (Y, X), representing the deformation fields."""
    ...

def set_from_fields(self, fields: tuple[...], weights: ...) -> None:
    """Sets the grid knots from dense deformation fields."""
    ...
```
...
- Use **float32** throughout for consistency and memory efficiency
- Sufficient for sub-pixel accuracy (~0.00001 pixel precision)
- Adequate for large images (4+ MP)

```python
# All arrays use float32
self._knots: tuple[NDArray[np.float32], NDArray[np.float32]] = ...
