...
from ..deform import Deformation, SplineGridHelper
...
class AbstractRegistration:
    ...
    def forward_mapping(self):
        ...
        if self.params.mapping.lower() == "forward":
            return True
        if self.params.mapping.lower() == "backward":
            return False
        raise ValueError("Invalid mapping.")

    def create_deformation(self, *args, **kwargs) -> Deformation:
        """Create a Deformation with the correct forward_mapping setting."""
        kwargs.setdefault("forward_mapping", self.forward_mapping)
        return Deformation(*args, **kwargs)
    ...
    def get_deform(self, i=0):
        ...
        Get the Deformation instance for image with index i. If groupwise
        registration was used, this deformation maps image i to the mean
        shape.
    ...
    def get_final_deform(self, i=0, j=1, mapping=None):
        ...
        Get the Deformation instance that maps image with index i
...
class BaseRegistration(AbstractRegistration):
    ...
    def _set_delta_deform(self, i, deform):
        ...
        if current is None:
            current = self.create_deformation(self._ims[0].ndim)
        ...
        if combine == "compose":
            ...
            raise RuntimeError("param combine_deforms is invalid: %s" % str(combine))
    ...
    def _register(self, verbose=1):
        ...
        # Check (sampling is always 1.0)
        pixel_scales = [final_scale / 1.0 for _ in range(self._ims[0].ndim)]
        ...
        self._pyramids = [ScaleSpacePyramid(im, final_scale) for im in self._ims]

        # Calculate max scale (sampling is always 1.0)
        ranges = [sh * 1.0 for sh in self._ims[0].shape]
...
class GDGRegistration(BaseRegistration):
    ...
    def _regularize_diffeomorphic(self, scale, deform, weight=None):
        ...
        Regularize the given Deformation in a way that makes it
        diffeomorphic. Returns the result as a Deformation.

        """
        # Check
        if not isinstance(deform, Deformation):
            raise ValueError("make_diffeomorphic needs a Deformation.")
        ...
        grid = SplineGridHelper.from_deformation(deform, grid_sampling, weight, injective, frozenedge)
        self.timer.stop("regularizing")

        return grid.to_deformation()
    ...
    def _deform_for_image_groupwise(self, i, iterInfo):
        ...
        # Init deform (empty = identity)
        totalDeform = Deformation(forward_mapping=self.forward_mapping)
        nims = len(self._ims)
        count = 0
