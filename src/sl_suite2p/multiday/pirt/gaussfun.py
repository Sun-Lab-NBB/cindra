...
def _gaussiankernel(sigma, order, t):
    ...
    if order == 0:
        ...
        part = -30240 + 302400 * x**2 - 403200 * x**4 + 161280 * x**6 - 23040 * x**8 + 1024 * x**10
    elif order == 11:
        part = -665280 * x + 2217600 * x**3 - 1774080 * x**5 + 506880 * x**7 - 56320 * x**9 + 2048 * x**11
    elif order == 12:
        part = (
            665280 - 7983360 * x**2 + 13305600 * x**4 - 7096320 * x**6 + 1520640 * x**8 - 135168 * x**10 + 4096 * x**12
        )
    elif order == 13:
        part = (
            17297280 * x
...
def gaussiankernel(sigma, order=0, N=None, returnt=False, warn=True):
    ...
    if warn and sigma < sigmaMin:
        print("WARNING: The scale (sigma) is very small for the given order, better use a larger scale!")
...
def gfilter2(L, scale, order=0, mode="reflect", warn=True):
    ...
    """
    # Filter (sampling is always 1.0, so sigma = scale)
    return gfilter(L, float(scale), order, mode, warn)
...
def diffuse2(L, scale, mode="nearest"):
    ...
    """
    # Filter (sampling is always 1.0, so sigma = scale)
    return diffuse(L, float(scale), mode)
