...
def resolve_parameters(configuration: SingleDayS2PConfiguration) -> RuntimeData:
    """Generates the output directory hierarchy and the 'single_day_ss2p_configuration.yaml' configuration file for the
    ...
    execution, this function can also be used between steps to save the configuration with updated runtime parameters
    to the YAML file.

    Args:
        configuration: A SingleDayS2PConfiguration instance with user-specified parameters to override the defaults with.
    ...
    if (
        isinstance(resolved_config.cellpose_detection.diameter, list)
        and len(resolved_config.cellpose_detection.diameter) > 1
        and resolved_config.output.aspect == 1.0
    ):
        resolved_config.output.aspect = (
            resolved_config.cellpose_detection.diameter[0] / resolved_config.roi_detection.diameter[1]
        )

    # If necessary, resolves the output (save folder) path, based on the input data path.
    if not resolved_config.output.save_path or len(resolved_config.output.save_path) == 0:
        if resolved_config.file_io.data_path and len(resolved_config.file_io.data_path) > 0:
            resolved_config.output.save_path = resolved_config.file_io.data_path[0]
        else:
            message = "The input data path and the save path are not specified."
            console.error(message=message, error=ValueError)

    # Ensures that the root save folder name is provided. Defaults to 'suite2p' if not provided.
    if not resolved_config.output.save_folder or len(resolved_config.output.save_folder) == 0:
        resolved_config.output.save_folder = "suite2p"

    # Actualizes version information to the SingleDatyS2PConfiguration instance.
    resolved_config.main.base_suite2p_version = version
    resolved_config.main.sl_suite2p_version = sl_version
    resolved_config.main.python_version = python_version

    # If the user does not specify the maximum parallel worker limit, set it based on available CPU cores
    if resolved_config.main.parallel_workers < 1:
        resolved_config.main.parallel_workers = os.process_cpu_count()

    # Ensures the root save directory exists, creating it, if necessary.
    save_folder = Path(resolved_config.output.save_path).joinpath(resolved_config.output.save_folder)
    save_folder.mkdir(parents=True, exist_ok=True)

    # Initializes a RuntimeData instance with the resolved configuration
    runtime_data = RuntimeData(configuration=resolved_config)
    runtime_data.save(output_directory=save_folder)

    return runtime_data


def run_s2p(runtime_data: RuntimeData) -> None:
    ...
    Args:
        ops_path: The path to the ops.npy file that stores the single-day suite2p processing parameters. Compatible
            ops.npy files are generated by the resolve_parameters() function.
    """
    save_folder = Path(runtime_data.configuration.output.save_path).joinpath(
        runtime_data.configuration.output.save_folder
    )
    runtime_yaml_path = save_folder.joinpath("runtime_data.yaml")

    if not runtime_yaml_path.exists() or not runtime_yaml_path.is_file():
        message = (
            f"Unable to run the single-day suite2p pipeline, as the 'runtime_data.yaml' file does not exist at the specified "
            f"path {runtime_yaml_path}."
        )
...
def process_plane(ops_path: Path, plane_index: int) -> None:
    ...
    Args:
        ops_path: The path to the ops.npy file used to store the suite2p processing parameters. Compatible ops.npy
            files are generated by the resolve_parameters() function.
    ...
    # and the 'resolve_parameters' function. This code was modified to also include 'nplanes', 'nrois', and 'nchannels' as
