...
def make_buttons(parent, b0):
    ...
    parent.probedit.returnPressed.connect(lambda: merge.apply(parent))
    ...
    parent.binedit.returnPressed.connect(lambda: parent.mode_change(parent.activityMode))
    ...
    for names in parent.color_names[:-3]:
        if b > 0:
            ...
            if b < len(parent.color_names) - 2:
                ...
                parent.colors["colorbar"].append([istat1, (istat99 - istat1) / 2 + istat1, istat99])
                istat = istat - istat1
...
def beh_masks(parent):
    ...
    parent.colors["cols"][c] = col
    parent.colors["istat"][c] = istat.flatten()
    parent.colors["colorbar"][c] = [istat_min, (istat_max - istat_min) / 2 + istat_min, istat_max]
    rgb_masks(parent, col, c)
...
def corr_masks(parent):
    ...
    cc[n] = cc.mean()
    istat = cc
    parent.colors["colorbar"][c] = [istat.min(), (istat.max() - istat.min()) / 2 + istat.min(), istat.max()]
    istat = istat - istat.min()
    istat = istat / istat.max()
    col = istat_transform(istat, parent.ops_plot["colormap"])
    parent.colors["cols"][c] = col
...
def remove_roi(parent, n, i0):
    ...
    parent.rois["LamNorm"][i0, ypix, xpix] = np.maximum(
        0, np.minimum(1, 0.75 * parent.rois["Lam"][i0, 0, ypix, xpix] / parent.rois["LamMean"])
    )
...
def add_roi(parent, n, i):
    ...
    parent.rois["LamNorm"][:, ypix, xpix] = np.maximum(
        0, np.minimum(1, 0.75 * parent.rois["Lam"][:, 0, ypix, xpix] / parent.rois["LamMean"])
    )
