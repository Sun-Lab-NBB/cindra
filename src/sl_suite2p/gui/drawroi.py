...
class ROIDraw(QMainWindow):
    def __init__(self, parent):
        ...
        self.Ly = self.parent.ops["frame_height"]
        self.Lx = self.parent.ops["frame_width"]
    ...
    def close_GUI(self):
        ...
        if "registered_binary_path_channel_2" in self.parent.ops:
    ...
    def normalize_img_add_masks(self):
        ...
        for i in np.arange(4):  # 4 because 4 buttons
            if i == 0:
                ...
                mimg[
                    self.parent.ops["valid_y_range"][0] : self.parent.ops["valid_y_range"][1],
                    self.parent.ops["valid_x_range"][0] : self.parent.ops["valid_x_range"][1],
                ...
                mimg[
                    self.parent.ops["valid_y_range"][0] : self.parent.ops["valid_y_range"][1],
                    self.parent.ops["valid_x_range"][0] : self.parent.ops["valid_x_range"][1],
                ] = self.parent.ops["correlation_map"]
                ...
                if "max_proj" in self.parent.ops:
                    mimg[
                        self.parent.ops["valid_y_range"][0] : self.parent.ops["valid_y_range"][1],
                        self.parent.ops["valid_x_range"][0] : self.parent.ops["valid_x_range"][1],
                    ] = self.parent.ops["maximum_projection"]
    ...
    def create_masks_of_cells(self, mean_img):
        ...
        pix = np.concatenate(((H[:, :, np.newaxis]), S[:, :, np.newaxis], mean_img[:, :, np.newaxis]), axis=-1)
        pix = hsv_to_rgb(pix)
        return pix
        ...
        for x in items:
            if x == self.img0:
                ...
                if event.modifiers() == QtCore.Qt.AltModifier:
                    self.add_ROI(pos=np.array([posx - 5, posy - 5, int(self.diam.text()), int(self.diam.text())]))
                if event.double():
    ...
    def proc_ROI(self):
        stat0 = []
        if self.extracted:
            for t, s in zip(self.scatter, self.tlabel, strict=False):
...
class sROI:
    def __init__(self, iROI, parent=None, pos=None, diameter=None, color=None, yrange=None, xrange=None):
    ...
    def position(self, parent):
        ...
        ellipse = (
            (y - br.center().y()) ** 2 / (br.plane_heights() / 2) ** 2
            + (x - br.center().x()) ** 2 / (br.plane_widths() / 2) ** 2
        ) <= 1
