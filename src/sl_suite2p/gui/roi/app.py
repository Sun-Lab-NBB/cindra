...
_ICON_PATH: str = str(MainWindow.suite2p_directory() / "logo" / "logo.png")
...
def run(session_path: Path | None = None) -> None:
    ...
    If a session path is provided, the viewer loads suite2p output data from that
    directory on startup.

    Args:
        session_path: Path to a suite2p output directory to load on startup.
    """
