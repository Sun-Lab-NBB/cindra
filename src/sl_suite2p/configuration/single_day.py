...
_PROCESSED_IMAGES = [
    ...
    "mean_image_channel_2_corrected",
    "max_projection",
    "correlation_map",
]
...
class IOData:
    ...
    height_range: list[int] = field(default_factory=list)
    """The pixel range to span the full height of the frame if registration is disabled."""

    width_range: list[int] = field(default_factory=list)
    ...
    mean_image_channel_2: NDArray[np.float32] | str | None = None
    ...
    plane_index: list[int] = field(default_factory=list)
...
class ROIDetectionData:
    ...
    max_projection: NDArray[np.int16] | str | None = None
    """The maximum-intensity projection of the binned movie computed along the time axis (axis 0)."""

    mean_image_channel_2_corrected: NDArray[np.float32] | str | None = None
...
class RuntimeData(YamlConfig):
    ...
    def __post_init__(self) -> None:
        ...
        if self.yaml_path is not None and isinstance(self.yaml_path, str):
