...
class IOData:
    ...
    height_range: NDArray[np.uint32] = field(default_factory=lambda: np.array([0, 0], dtype=np.uint32))
    """The pixel range to span the full height of the frame if registration is disabled."""

    width_range: NDArray[np.uint32] = field(default_factory=lambda: np.array([0, 0], dtype=np.uint32))
...
class RuntimeData(YamlConfig):
    ...
    def save(self) -> None:
        ...
        Args:
            output_directory: The path to the directory where the .npy image files and YAML file are saved.
        """
        if self.yaml_path is None:
            message = "The runtime_data.yaml file does not exist for the specific plane"
        ...
        # Path objects
        if original.yaml_path is not None:
