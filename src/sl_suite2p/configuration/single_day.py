...
def serialize_nparrays(obj: Any, to_list: bool = False) -> None:
    """Recursively converts data between numpy arrays and lists.

    This service function ensures that any runtime data fields stored as numpy arrays are converted to lists before
    saving to a YAML file. When loading from YAML, those lists are converted back into numpy arrays using their
    original data types.

    Args:
        obj: The dataclass instance or object containing the data to convert.
        to_list: Whether to convert numpy arrays to lists (True) or restore lists back to numpy arrays (False).
    """
    if obj is None:
        return
    ...
    for attr in list(vars(obj)):
        ...
        if hasattr(value, "__dataclass_fields__"):
            serialize_nparrays(value, to_list)

        if to_list and isinstance(value, np.ndarray):
            # Stores the dtype info as a separate attribute
            dtype_attr = f"_{attr}_dtype"
            setattr(obj, dtype_attr, str(value.dtype))
            setattr(obj, attr, value.tolist())

        elif not to_list and isinstance(value, list):
            # Restores the original numpy with the stored dtype
            dtype_attr = f"_{attr}_dtype"
            stored_dtype = getattr(obj, dtype_attr, None)
            if stored_dtype:
                dtype = np.dtype(stored_dtype)
                setattr(obj, attr, np.array(value, dtype=dtype))

                # Cleans up the temporary dtype attribute
                delattr(obj, dtype_attr)
...
@dataclass
class IOData:
    """Stores the output data and parameters generated by the IO module for each plane."""
    ...
    reg_file: str | Path = Path()
    """Stores the absolute path to the (functional) channel 1 memory-mapped binary file."""

    reg_file_channel_2: str | Path = Path()
    """Stores the absolute path to the (functional) channel 2 memory-mapped binary file."""

    raw_file: str | Path = Path()
    """Stores the absolute path to the raw channel 1 memory-mapped binary file. The raw file is only
    saved when keep_movie_raw=True."""

    raw_file_channel_2: str | Path = Path()
    """Stores the absolute path to the raw channel 2 memory-mapped binary file."""

    mean_image: NDArray[np.float32] | None = None
    """The mean image computed as the pixel-wise average intensity across all frames in the channel 1 recording."""

    enhanced_mean_image: NDArray[np.float32] | None = None
    """The enhanced mean image created by subtracting a median-filtered background, normalizing by the local variance
    and clipping the pixel intensities to a fixed range."""
    ...
    """Stores the x-coordinates for all ROI-plane combinations, where each ROI's horizontal position is repeated across 
    all planes."""

    plane_index: NDArray[np.int32] = field(default_factory=lambda: np.array([], dtype=np.int32))
    """Stores the plane index for each ROI in the re-arranged data to represent all ROI by plane combinations."""
...
# FIXME: placeholder for refactoring the 'combined' function in save.py
@dataclass
class ROIDetectionData:
    """Stores the output data generated by the detection module for each ROI."""

    max_projection: NDArray[np.int16] | None = None
    """The maximum-intensity projection of the binned movie computed along the time axis (axis 0)."""

    mean_image_channel_2_corrected: NDArray[np.float32] | None = None
    """The bleedthrough-corrected channel 2 mean image. The estimated green-to-red bleedthrough contribution 
...
class SpikeDeconvolution:
    ...
    """The standard deviation, in seconds, of the Gaussian filter used to compute the 'maximin' and 'constant' 
    baselines."""

    baseline_percentile: float = 8.0
    """The (lowest) percentile of each trace's activity used as the 'constant_percentile' baseline."""
...
@dataclass
class RuntimeData(YamlConfig):
    """Stores all default configuration parameters from the SingleDayS2PConfiguration class and the added data from the
    SingleDayS2PData class.
    """

    configuration: SingleDayS2PConfiguration = field(default_factory=SingleDayS2PConfiguration)
    data: SingleDayS2PData = field(default_factory=SingleDayS2PData)
    ...
    def __post_init__(self) -> None:
        ...
        self.data.file_io.raw_file = Path(self.data.file_io.raw_file)
        ...
        serialize_nparrays(self, to_list=False)

    def set_yaml_path(self, plane_index: int) -> None:
        """Creates the plane-specific directory for the given plane index and sets yaml_path to the
        runtime_data.yaml file inside it."
    ...
    def save(self) -> None:
        ...
        # Converts all Path objects storing image data to strings before dumping the data, as .YAML encoder does not recognize Path objects.
        original.yaml_path = str(original.yaml_path) if original.yaml_path else None
        ...
        # Converts all numpy arrays in the RuntimeData object to lists.
        serialize_nparrays(original, to_list=True)

        # Saves the mean_image, enhanced_mean_image, and mean_image_channel_2 into separate .npy files if they exist.
        # These attributes are reset to None before dumping into YAML.
        processed_images = ["mean_image", "enhanced_mean_image", "mean_image_channel_2"]

        for image in processed_images:
            image_data = getattr(original.data.file_io, image, None)
            if image_data is not None:
                image_file_path = plane_directory.joinpath(f"{image}.npy")
                np.save(image_file_path, image_data)
                setattr(original.data.file_io, image, None)

        # Saves the data to the yaml path
        original.to_yaml(file_path=self.yaml_path)

    def to_dict(self) -> dict[str, Any]:
        """Returns the instance as a dictionary when saving the combined data as a MATLAB file."""
        combined_dict = {}
