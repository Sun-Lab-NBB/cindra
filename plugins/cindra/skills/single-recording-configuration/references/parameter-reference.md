...
Documents the nine configuration sections, listing every parameter with its type, default, and meaning, along with the
tuning guidance each section carries.
...
Worker allocation reaches each stage as an invocation argument, as the skill's Configuration overview section describes.
The measured defaults are published as `BINARIZATION_WORKERS`, `REGISTRATION_WORKERS`, `REGISTRATION_GPU_WORKERS`,
...
extracted fluorescence traces, so it only joins the feature set during the final classification in Section 8. The
threshold defaults to zero, because the filter decides on shape alone and discards permanently, so a region it rejects
never reaches the classifier that would have judged it on its activity.
